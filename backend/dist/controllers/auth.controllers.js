...
export const verifyOtp = async (req, res) => {
    try {
        ...
        const updatedUser = await userModel.findByIdAndUpdate(otpRecord.user, { isVerified: true }, { returnDocument: "after" });
        if (!updatedUser) {
        ...
        const refreshToken = jwt.sign({ id: updatedUser._id }, config.Jwt_Secret, { expiresIn: config.Jwt_Refresh_Expires_In });
        ...
        const accessToken = jwt.sign({ id: updatedUser._id, sessionId: session._id }, config.Jwt_Secret, { expiresIn: config.Jwt_Access_Expires_In });
...
export const login = async (req, res) => {
    try {
        ...
        const refreshToken = jwt.sign({ id: user._id }, config.Jwt_Secret, { expiresIn: config.Jwt_Refresh_Expires_In });
        const refreshTokenHash = hashToken(refreshToken);
        ...
        const accessToken = jwt.sign({ id: user._id, sessionId: session._id }, config.Jwt_Secret, { expiresIn: config.Jwt_Access_Expires_In });
...
export const refreshToken = async (req, res) => {
    try {
        ...
        const newRefreshToken = jwt.sign({ id: decoded.id }, config.Jwt_Secret, { expiresIn: config.Jwt_Refresh_Expires_In });
        const newRefreshTokenHash = hashToken(newRefreshToken);
        // Update session
        session.refreshTokenHash = newRefreshTokenHash;
        await session.save();
        const newAccessToken = jwt.sign({ id: decoded.id, sessionId: session._id }, config.Jwt_Secret, { expiresIn: config.Jwt_Access_Expires_In });
        // Set rotated cookie
...
export const logout = async (req, res) => {
    try {
        ...
        if (token) {
            const incomingHash = hashToken(token);
            await SessionModel.findOneAndUpdate({ refreshTokenHash: incomingHash }, { revoked: true });
        }
        if (req.sessionId) {
...
export const updateProfile = async (req, res) => {
    try {
        ...
        const { username, profession, fullName, phone, location, bio, targetRole, linkedinUrl, githubUrl, portfolioUrl, skills, } = req.body;
        const updateData = {};
