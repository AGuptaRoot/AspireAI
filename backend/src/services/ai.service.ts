...
export const getOrCreateVectorStore = async (
    chunks: string[],
    embeddings: GoogleGenerativeAIEmbeddings | null,
    resumeId?: string
 ...
 */
export const warmResumeVectorStore = async (
    chunks: string[],
    resumeId?: string
): Promise<void> => {
...
): Promise<{ answer: string; relevantChunks: string[]; resultText: string; mode: string }> => {
    let result = "";
...
export async function* streamCareerAssistantResponse(
    ...
    mode: "fast" | "deep" = "fast",
    rawResumeText?: string
): AsyncGenerator<{ text?: string; relevantChunks?: string[]; resultText?: string; done?: boolean; mode?: string }, void, unknown> {
    const isDeepMode = mode === "deep";
    let result = "";
    let relevantChunks: string[] = [];
